...
///// File selector 
  Button b1 = cp5.addButton("selectFile")
    .setValue(0)
      .setPosition(100, 100)
        .setSize(200, 19)
  ...
  if (millis()-lastFileRead > 500 ) {
    try {
      ...
      for (int i=0; i<valArray.length; i++) {
        ...
        if (normalize==true){
          mensaje.add(f);
          println(direccionOsc[i], f);
        } 
        else {
          float v = map(f, mins[i], maxs[i], 0, 1);
          mensaje.add(v);
          println(direccionOsc[i], v);
        }
...
/*
190830 neoCSVReaderOSC
 .direct callback for b1
 .normalizable button
